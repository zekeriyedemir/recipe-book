...
function RecipeCard({ recipe: { title, duration, category, image, servings, description } }: { recipe: Recipe }) {
  return (
    <article className="card overflow-hidden bg-base-100 shadow-md transition-shadow hover:shadow-lg">
      ...
      <div className="card-body">
        <div className="flex items-start justify-between gap-3"><h2 className="card-title">{title}</h2>{category && <span className="badge badge-primary badge-outline">{category}</span>}</div>
